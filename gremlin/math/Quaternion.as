package gremlin.math
{
	...
	public class Quaternion
	{
		...
		public function multiplyBy(b:Quaternion):void {
			this.setTo(values.x * b.values.w + values.w * b.values.x + values.y * b.values.z - values.z * b.values.y,
				values.y * b.values.w + values.w * b.values.y + values.z * b.values.x - values.x * b.values.z,
				values.z * b.values.w + values.w * b.values.z + values.x * b.values.y - values.y * b.values.x,
				values.w * b.values.w - values.x * b.values.x - values.y * b.values.y - values.z * b.values.z
			);
		}
		...
		public function transformVector(vec:Vector3D):void {
			...
			vec.setTo(
				ix * values.w + iw * -values.x + iy * -values.z - iz * -values.y,
				iy * values.w + iw * -values.y + iz * -values.x - ix * -values.z,
				iz * values.w + iw * -values.z + ix * -values.y - iy * -values.x
			);
		}
		...
		public function toAngleAxis(v:Vector3D):void
		{
			if (1.0 - values.w * values.w != 0) {
				v.setTo(
					values.x / Math.sqrt(1.0 - values.w * values.w),
					values.y / Math.sqrt(1.0 - values.w * values.w),
					values.z / Math.sqrt(1.0 - values.w * values.w)
				);
